...
function PageThumb({ page, palette }: { page: GeneratedPage | null; palette: Palette }) {
  ...
  return (
    <div className="animate-pop overflow-hidden rounded-2xl border-[2.5px] border-ink shadow-block-sm">
...
export function ProgressScreen({
  steps, log, latest, palette, onCancel, title,
}: {
    ...
    <div className="mx-auto grid w-full max-w-4xl gap-6 lg:grid-cols-[1fr_300px]">
      <section className="card-paper step-enter p-6 sm:p-8">
        ...
        <p className="mt-1.5 text-sm font-semibold text-ink/60">
          Пайплайн из восьми модулей идёт по порядку — каждый шаг пишет в журнал.
        </p>
          ...
          {steps.map((s) => (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className={cx("block font-display text-[14.5px] font-bold", s.status === "pending" ? "text-ink/40" : "text-pine")}>
                  {s.label}
                </span>
        ...
        <div className="mt-6 flex items-center justify-between gap-3">
          <span className="font-display text-[13px] font-bold text-ink/55">
            готово {doneCount} из {steps.length}
          </span>
