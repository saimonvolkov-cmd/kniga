...
function SourceSticker({ page }: { page: GeneratedPage }) {
  const base = "absolute left-3 top-3 z-10 -rotate-2 rounded-lg border-2 border-ink px-2 py-1 text-[10.5px] font-extrabold uppercase tracking-wide shadow-block-sm";
  if (page.imageSource === "demo")
    return (
      <span title={page.imageError} className={cx(base, "bg-coral text-paper")}>
        демо{page.imageError ? ` · ${shortError(page.imageError)}` : ""}
      </span>
    );
  if (page.imageSource === "yandex-art") return <span className={cx(base, "bg-marigold text-pine")}>YandexART</span>;
  if (page.imageSource === "huggingface") return <span className={cx(base, "bg-berry text-paper")}>Hugging Face</span>;
  if (page.imageSource === "gemini") return <span className={cx(base, "bg-sea text-paper")}>Gemini</span>;
  return null;
}
...
function BookPage({ page, book, palette, index }: { page: GeneratedPage; book: GeneratedBook; palette: Palette; index: number }) {
  ...
  return (
    <figure ref={ref} className={cx("page-reveal mx-auto w-full max-w-[600px]", index % 2 === 0 ? "md:-rotate-[0.6deg]" : "md:rotate-[0.6deg]")}>
      ...
      <header className="step-enter mb-8 text-center">
        ...
        {rep && (
          <p className="mt-3 text-[12.5px] font-extrabold text-ink/55">
            иллюстрации: <span className="text-marigold">YandexART {rep.yandex}</span> ·{" "}
            <span className={rep.gemini ? "text-sea" : "text-ink/40"}>Gemini {rep.gemini}</span> ·{" "}
...
{rep.firstError}
            ...
            <p className="mt-2 text-[12.5px] font-bold leading-snug text-ink/60">
              Проверьте генерацию в панели «Тест генерации» или запустите прокси «npm --prefix server run server».
            </p>
          </div>
        )}
