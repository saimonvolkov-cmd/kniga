...
import {
  AGE_GROUPS, COMPANION_ROLES, COMPANION_TYPES, GENRES, PALETTES, SPREAD_COUNTS, THEMES,
} from "../data/content";
import { downscaleImage } from "../lib/utils";
import { cx, ChunkyButton, OptionCard, SectionHead, Sticker } from "./ui";
import {
  ...
  return (
    <div className="rounded-2xl border-[2.5px] border-ink bg-foam p-4">
      ...
      >
        ...
        />
        <p className="mt-2 text-center text-[11px] font-semibold text-ink/45">
...
export function PhotoStep({ draft, patch, toast }: StepProps) {
  return (
    <div>
      <SectionHead
        step="Шаг 1 · референсы"
        title="Кто будет в книге?"
        subtitle="Загрузите фото ребёнка — иллюстратор сохранит образ. Фото питомца или игрушки — по желанию."
      />
      <div className="grid gap-4 sm:grid-cols-2">
        <PhotoDrop
          title="Фото ребёнка" note="1–3 портрета, анфас или профиль" max={3}
          photos={draft.childPhotos} onChange={(v) => patch({ childPhotos: v })} toast={toast} accent="#f2b33d"
        />
        <PhotoDrop
          title="Питомец или игрушка" note="необязательно — попадёт на страницы" max={2}
          photos={draft.companionPhotos} onChange={(v) => patch({ companionPhotos: v })} toast={toast} accent="#3f8f8a"
        />
      </div>
...
const THEME_ICONS = {
  ...
  darkness: IconMoon,
  sharing: IconHeart,
  sibling_jealousy: IconUsers,
  trying_new: IconSprout,
  surprise_me: IconDice,
} as const;
      ...
      <div className="grid max-w-2xl gap-4">
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => patch({ favoritePlace: e.target.value })}
            placeholder="Например: бабушкина дача"
            maxLength={40}
            className="field-input w-full px-4 py-3 font-bold text-pine"
          />
        </label>
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => patch({ favoriteActivity: e.target.value })}
            placeholder="Например: рисовать динозавров"
            maxLength={40}
            className="field-input w-full px-4 py-3 font-bold text-pine"
          />
        </label>
...
export function StepNav({
  ...
  return (
    <div className="mt-7 flex flex-wrap items-center gap-3 border-t-2 border-dashed border-ink/20 pt-5">
      {onBack && (
        <ChunkyButton variant="ghost" onClick={onBack}>Назад</ChunkyButton>
      )}
      <ChunkyButton onClick={onNext} disabled={!canNext}>
        {nextLabel ?? "Дальше"}
      </ChunkyButton>
