...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   Правила из ТЗ соблюдены: персонаж никогда не смотрит в камеру — зрачки,
   наклон корпуса и поднятая рука всегда направлены на объект действия;
   фон заполнен деталями; палитра — приглушённая, цвета палитры как акценты.
   ──────────────────────────────────────────────────────────────────────── */
...
/* ── небесные и фоновые слои по жанрам ──────────────────────────────────── */
function starDots(rnd: () => number, count: number, maxY: number, paper: string) {
  return scatter(rnd, count, 0, 0, 1080, maxY)
    .map((s) => `<circle cx="${s.x}" cy="${s.y}" r="${s.r}" fill="${paper}" opacity="${s.o}"/>`)
    .join("");
}

function environment(g: Genre, rnd: () => number, P: Palette["colors"], dark: boolean): string {
  const [INK, DEEP, MID, LIGHT, PAPER, WARM, COOL] = P;
  const s: string[] = [];
  switch (g) {
    case "forest": {
      ...
      const pines = scatter(rnd, 9, 30, 470, 1050, 640);
      for (const t of pines) {
        const h = 90 + rnd() * 90;
        s.push(
          `<g transform="translate(${t.x} ${t.y})">` +
          `<rect x="-5" y="-12" width="10" height="26" fill="${INK}" opacity="0.8"/>` +
          `<path d="M0 ${-h} L${26 + h * 0.16} 0 L${-(26 + h * 0.16)} 0 Z" fill="${DEEP}"/>` +
          `<path d="M0 ${-h * 0.72} L${20 + h * 0.12} ${-h * 0.08} L${-(20 + h * 0.12)} ${-h * 0.08} Z" fill="${dark ? DEEP : MID}"/>` +
          `</g>`
        );
      }
    ...
    case "space": {
      s.push(starDots(rnd, 60, 1080, PAPER));
      s.push(
        `<ellipse cx="540" cy="520" rx="520" ry="110" fill="${LIGHT}" opacity="0.1" transform="rotate(-18 540 520)"/>`
      );
      s.push(
        ...
        `<circle cx="818" cy="188" r="14" fill="${PAPER}" opacity="0.5"/><circle cx="882" cy="242" r="9" fill="${PAPER}" opacity="0.4"/>`
      );
      s.push(
    ...
    case "fairy_tale": {
      ...
      s.push(
        ...
        `<path d="M-46 0 L0 -78 L46 0 Z" fill="${COOL}"/><rect x="-10" y="36" width="20" height="30" rx="9" fill="${INK}" opacity="0.75"/>` +
        `<line x1="0" y1="-78" x2="0" y2="-106" stroke="${INK}" stroke-width="4"/><path d="M0 -106 L30 -98 L0 -88 Z" fill="${WARM}"/></g>`
      );
    ...
    case "underwater": {
      s.push(
        `<polygon points="300,-10 420,-10 760,1090 560,1090" fill="${PAPER}" opacity="0.08"/>` +
        `<polygon points="560,-10 640,-10 1000,1090 860,1090" fill="${PAPER}" opacity="0.06"/>`
      );
      for (let i = 0; i < 6; i++) {
    ...
    case "pirates": {
      ...
      s.push(
        `<g transform="translate(820 512)"><path d="M-64 0 Q0 30 64 0 L46 -6 L-46 -6 Z" fill="${INK}"/>` +
        `<line x1="0" y1="-6" x2="0" y2="-86" stroke="${INK}" stroke-width="6"/>` +
        `<path d="M0 -86 Q44 -74 6 -40 L0 -40 Z" fill="${PAPER}" opacity="0.9"/></g>`
      );
      s.push(`<path d="M300 300 q14 -14 28 0 q14 -14 28 0" fill="none" stroke="${INK}" stroke-width="5" stroke-linecap="round" opacity="0.7"/>`);
      break;
    }
    case "superhero": {
      s.push(starDots(rnd, dark ? 40 : 12, 420, PAPER));
      const bldgs = scatter(rnd, 9, 20, 0, 1060, 40);
      let x = -30;
      bldgs.forEach((b) => {
        const w = 90 + b.r * 34;
        const h = 240 + rnd() * 320;
        s.push(`<rect x="${R(x)}" y="${R(620 - h)}" width="${R(w)}" height="${R(h)}" fill="${dark ? INK : MID}" opacity="${dark ? 0.95 : 0.85}"/>`);
      ...
      s.push(`<rect x="0" y="620" width="1080" height="460" fill="${dark ? INK : DEEP}"/>`);
      break;
    }
  }
  return s.join("");
...
function groundBand(g: Genre, P: Palette["colors"]): string {
  ...
  return `<path d="M-20 884 ${bumps.join(" ")} L1120 1100 L-20 1100 Z" fill="${g === "space" ? INK : DEEP}"/>` + deco.join("");
}

/* ── фокальный объект (то, на что направлен взгляд) ─────────────────────── */
function focalObject(id: FocalId, x: number, y: number, P: Palette["colors"], rnd: () => number): string {
  ...
  switch (id) {
    ...
    case "planet":
      ...
      break;
    case "moon":
      body = `<circle r="44" fill="${PAPER}"/><circle cx="-16" cy="-12" r="40" fill="${LIGHT}" opacity="0"/><path d="M14 -44 A44 44 0 1 0 44 12 A34 34 0 1 1 14 -44 Z" fill="${WARM}" opacity="0.9"/>`;
      break;
    case "lantern":
      ...
      break;
    case "butterfly": {
      const w = `<path d="M0 0 Q-46 -40 -56 -8 Q-60 22 -8 12 Z" fill="${COOL}"/><path d="M0 0 Q46 -40 56 -8 Q60 22 8 12 Z" fill="${WARM}"/><ellipse rx="5" ry="16" fill="${PAPER}"/>`;
      body = w;
      break;
    }
    case "mushroom":
    ...
    case "flower":
      for (let i = 0; i < 6; i++)
        body += `<ellipse cx="0" cy="-26" rx="13" ry="26" fill="${COOL}" transform="rotate(${i * 60})"/>`;
    ...
    case "kite":
      ...
      break;
    case "bird":
      body = `<ellipse rx="26" ry="18" fill="${PAPER}"/><circle cx="22" cy="-12" r="13" fill="${PAPER}"/><path d="M34 -12 l12 4 l-12 5 Z" fill="${WARM}"/><path d="M-8 -2 Q-28 -14 -20 4 Q-10 10 -8 -2 Z" fill="${COOL}"/><circle cx="26" cy="-14" r="2.6" fill="${PAPER}"/>`;
      break;
    case "bell":
      ...
      break;
    case "key":
      body = `<circle cx="0" cy="-24" r="20" fill="none" stroke="${WARM}" stroke-width="10"/><line x1="0" y1="-4" x2="0" y2="46" stroke="${WARM}" stroke-width="10" stroke-linecap="round"/><line x1="0" y1="34" x2="18" y2="34" stroke="${WARM}" stroke-width="9" stroke-linecap="round"/><line x1="0" y1="46" x2="14" y2="46" stroke="${WARM}" stroke-width="9" stroke-linecap="round"/>`;
      break;
    case "lighthouse":
...
function faceMouth(emotion: string, P: Palette["colors"]): string {
  ...
  if (/восторг|радость|счаст/.test(em))
    return `<path d="M-13 0 Q0 16 13 0 Z" fill="${INK}"/>`;
  ...
  if (/решим/.test(em))
    return `<line x1="-10" y1="2" x2="10" y2="2" stroke="${INK}" stroke-width="4.5" stroke-linecap="round"/>`;
...
function childFigure(opts: {
  ...
  emotion: string; P: Palette["colors"]; flip?: boolean;
}): string {
  ...
  const px = (dirX / len) * 3.4, py = (dirY / len) * 3.4; // зрачки к фокусу
  const lean = Math.max(-7, Math.min(7, dirX / 90)); // наклон корпуса к действию
  // поднятая рука к фокусу
  ...
  const hair =
    gender === "male"
      ...
      : `<path d="M-50 -150 Q-54 -210 0 -210 Q54 -210 50 -150 L50 -128 Q44 -150 34 -158 Q10 -178 -34 -158 Q-44 -150 -50 -128 Z" fill="${DEEP}"/>` +
        `<circle cx="34" cy="-176" r="9" fill="${WARM}"/>`;
  const eyes = [-15, 15]
    .map(
      (ex) =>
        `<ellipse cx="${ex}" cy="-156" rx="7" ry="8" fill="${PAPER}"/><circle cx="${R(ex + px)}" cy="${R(-156 + py)}" r="3.6" fill="${INK}"/>`
    )
    .join("");
  const brows = /волн|растер/.test(emotion.toLowerCase())
    ? `<line x1="-22" y1="-174" x2="-8" y2="-170" stroke="${INK}" stroke-width="4" stroke-linecap="round"/><line x1="8" y1="-170" x2="22" y2="-174" stroke="${INK}" stroke-width="4" stroke-linecap="round"/>`
    : `<path d="M-22 -172 Q-15 -177 -8 -173 M8 -173 Q15 -177 22 -172" fill="none" stroke="${INK}" stroke-width="4" stroke-linecap="round"/>`;
  return (
    ...
    eyes + brows + `<g transform="translate(0 -135)">${faceMouth(emotion, P)}</g>` +
    `</g>`
  );
}
...
function companionFigure(kind: CompanionKind, x: number, y: number, s: number, focalX: number, focalY: number, P: Palette["colors"]): string {
  ...
  switch (kind) {
    case "cat":
      body = `<ellipse cy="-34" rx="30" ry="26" fill="${WARM}"/><circle cy="-72" r="22" fill="${WARM}"/>` +
        `<path d="M-18 -84 L-14 -102 L-2 -88 Z" fill="${WARM}"/><path d="M18 -84 L14 -102 L2 -88 Z" fill="${WARM}"/>` +
        eyes(0, -74) + `<path d="M-4 -64 Q0 -60 4 -64" fill="none" stroke="${DEEP}" stroke-width="2.6" stroke-linecap="round"/>` +
        `<path d="M28 -30 Q54 -44 50 -70" fill="none" stroke="${WARM}" stroke-width="9" stroke-linecap="round"/>` +
        `<path d="M-12 -52 h24 M-12 -44 h24" stroke="${DEEP}" stroke-width="3" opacity="0.5"/>`;
      break;
    case "dog":
      body = `<ellipse cy="-32" rx="30" ry="25" fill="${LIGHT}"/><circle cy="-70" r="22" fill="${LIGHT}"/>` +
        `<ellipse cx="-20" cy="-80" rx="9" ry="16" fill="${DEEP}" transform="rotate(18 -20 -80)"/>` +
        `<ellipse cx="20" cy="-80" rx="9" ry="16" fill="${DEEP}" transform="rotate(-18 20 -80)"/>` +
        eyes(0, -72) + `<ellipse cy="-58" rx="8" ry="6" fill="${DEEP}"/>` +
        `<path d="M-34 -22 Q-52 -30 -50 -48" fill="none" stroke="${LIGHT}" stroke-width="9" stroke-linecap="round"/>`;
      break;
    case "bunny":
      body = `<ellipse cy="-32" rx="27" ry="25" fill="${PAPER}"/><circle cy="-68" r="20" fill="${PAPER}"/>` +
        `<ellipse cx="-9" cy="-100" rx="7" ry="22" fill="${PAPER}"/><ellipse cx="9" cy="-100" rx="7" ry="22" fill="${PAPER}"/>` +
        `<ellipse cx="-9" cy="-100" rx="3" ry="14" fill="${WARM}" opacity="0.6"/><ellipse cx="9" cy="-100" rx="3" ry="14" fill="${WARM}" opacity="0.6"/>` +
        eyes(0, -70) + `<circle cy="-60" r="3.4" fill="${WARM}"/>`;
      break;
    case "bear":
      body = `<ellipse cy="-34" rx="30" ry="27" fill="${COOL}"/><circle cy="-74" r="23" fill="${COOL}"/>` +
        `<circle cx="-16" cy="-92" r="9" fill="${COOL}"/><circle cx="16" cy="-92" r="9" fill="${COOL}"/>` +
        eyes(0, -76) + `<ellipse cy="-64" rx="9" ry="7" fill="${PAPER}"/><circle cy="-66" r="3" fill="${DEEP}"/>`;
      break;
    case "robot":
      body = `<rect x="-24" y="-58" width="48" height="48" rx="10" fill="${COOL}"/><rect x="-19" y="-92" width="38" height="30" rx="8" fill="${LIGHT}"/>` +
        `<circle cx="-8" cy="-78" r="5" fill="${DEEP}"/><circle cx="8" cy="-78" r="5" fill="${DEEP}"/>` +
        `<line x1="0" y1="-92" x2="0" y2="-106" stroke="${COOL}" stroke-width="4"/><circle cy="-110" r="5" fill="${WARM}"/>` +
        `<rect x="-10" y="-46" width="20" height="12" rx="5" fill="${WARM}"/>`;
      break;
    case "fox":
      body = `<ellipse cy="-32" rx="28" ry="24" fill="${WARM}"/><circle cy="-68" r="21" fill="${WARM}"/>` +
        `<path d="M-18 -78 L-16 -98 L-4 -84 Z" fill="${WARM}"/><path d="M18 -78 L16 -98 L4 -84 Z" fill="${WARM}"/>` +
        `<ellipse cy="-58" rx="8" ry="6" fill="${PAPER}"/>` + eyes(0, -72) +
        `<path d="M30 -26 Q56 -34 58 -58 Q58 -36 44 -22 Q36 -16 30 -26 Z" fill="${WARM}"/><path d="M58 -58 Q57 -44 48 -36 Q56 -46 58 -58 Z" fill="${PAPER}"/>`;
      break;
  }
  return `<g transform="translate(${x} ${y}) scale(${s})">${body}</g>`;
}

const COMPANION_KINDS: CompanionKind[] = ["cat", "dog", "bunny", "bear", "robot", "fox"];
...
export function renderScene(spec: SceneSpec): string {
  ...
  const skyBot = night ? DEEP : spec.genre === "underwater" ? MID : LIGHT;

  // позиции: персонаж с одной стороны, фокус — с другой, взгляд всегда в сцену
  ...
  parts.push(
    ...
    `<filter id="grain" x="0" y="0" width="100%" height="100%"><feTurbulence type="fractalNoise" baseFrequency="0.82" numOctaves="2" stitchTiles="stitch"/><feColorMatrix type="saturate" values="0"/></filter>` +
    `</defs>`
  );
  ...
  parts.push(focalObject(spec.focal, R(fx), R(fy), P, rnd));
  // искры у фокуса
  for (let i = 0; i < 5; i++) {
  ...
  parts.push(
    childFigure({
      x: cx, y: cy, scale: spec.isCover ? 1.28 : 1.06, gender: spec.gender,
      focalX: fx, focalY: fy, emotion: spec.emotion, P,
    })
  );
...
export function renderCoverScene(input: BookInput, palette: Palette, seed: number, kind: CompanionKind | null): string {
  ...
  return renderScene({
    ...
    emotion: "восторг",
    focal: pick(rnd, GENRE_FOCALS[input.genre]),
    spreadNumber: 0,
    isCover: true,
  });
}
...
export function renderBackScene(input: BookInput, palette: Palette, seed: number): string {
  ...
  const [INK, DEEP, , , PAPER, WARM, COOL] = P;
  const tiles: string[] = [];
  ...
  for (let gy = 0; gy < 5; gy++)
    for (let gx = 0; gx < 5; gx++) {
      ...
      tiles.push(
        ...
        focalObject(id, 0, 0, P, rnd).replace(/translate\(([^)]+)\)/, "") + `</g>`
      );
    }
  return (
...
/* ── промпт для реального Gemini-вызова (референс-фото прикладывает api.ts) ─ */
