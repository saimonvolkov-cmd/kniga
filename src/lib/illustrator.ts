...
/* ─────────────────────────────────────────────────────────────────────────
   Illustration Module (демо-движок). Рисует «рукотворную» книжную сцену
   1080×1080 как self-contained SVG. Правила из ТЗ соблюдены: персонаж
   никогда не смотрит в камеру — зрачки, наклон корпуса и поднятая рука
   направлены на объект действия; фон заполнен деталями; палитра приглушённая.
   ──────────────────────────────────────────────────────────────────────── */
...
const GENRE_FOCALS: Record<Genre, FocalId[]> = {
  ...
  fairy_tale: ["bell", "key", "moon", "mushroom", "cloud", "dragon"],
...
function environment(g: Genre, rnd: () => number, P: Palette["colors"], dark: boolean): string {
  ...
  switch (g) {
    case "forest": {
      ...
      scatter(rnd, 9, 30, 470, 1050, 640).forEach((t) => {
        const h = 90 + rnd() * 90;
        s.push(
          `<g transform="translate(${t.x} ${t.y})"><rect x="-5" y="-12" width="10" height="26" fill="${INK}" opacity="0.8"/>` +
    ...
    case "superhero": {
      ...
      let x = -30;
      scatter(rnd, 9, 20, 0, 1060, 40).forEach((b) => {
        const w = 90 + b.r * 34;
        const h = 240 + rnd() * 320;
      ...
      s.push(`<g transform="translate(250 180)"><ellipse rx="80" ry="26" fill="${PAPER}" opacity="0.9"/><ellipse cx="52" cy="10" rx="52" ry="20" fill="${PAPER}" opacity="0.75"/></g>`);
      s.push(`<g transform="translate(800 130)"><ellipse rx="64" ry="22" fill="${PAPER}" opacity="0.8"/></g>`);
...
function focalObject(id: FocalId, x: number, y: number, P: Palette["colors"]): string {
  ...
  switch (id) {
    ...
    case "kite":
      ...
      break;
    case "bird":
      body = `<ellipse rx="26" ry="18" fill="${PAPER}"/><circle cx="22" cy="-12" r="13" fill="${PAPER}"/><path d="M34 -12 l12 4 l-12 5 Z" fill="${WARM}"/><path d="M-8 -2 Q-28 -14 -20 4 Q-10 10 -8 -2 Z" fill="${COOL}"/><circle cx="26" cy="-14" r="2.6" fill="${COOL}"/>`;
      break;
    case "bell":
    ...
    case "lighthouse":
      ...
      break;
    case "dragon":
      body = `<ellipse rx="46" ry="34" fill="${COOL}"/><circle cx="36" cy="-24" r="22" fill="${COOL}"/><path d="M28 -44 L36 -58 L44 -44 Z" fill="${COOL}"/><path d="M-40 -6 Q-66 -20 -70 4 Q-58 16 -40 8 Z" fill="${COOL}"/><circle cx="30" cy="-28" r="3.4" fill="${PAPER}"/><path d="M52 -18 q10 2 6 10" fill="none" stroke="${WARM}" stroke-width="5" stroke-linecap="round"/><path d="M-20 -34 q4 -10 12 -8 M0 -38 q4 -10 12 -8" stroke="${LIGHT}" stroke-width="5" fill="none" stroke-linecap="round"/>`;
      break;
  }
...
function childFigure(opts: {
  x: number; y: number; scale: number; gender: Gender; focalX: number; focalY: number; emotion: string; P: Palette["colors"];
}): string {
  const { x, y, scale: s, gender, focalX, focalY, emotion, P } = opts;
  const [INK, DEEP, MID, LIGHT, PAPER, WARM] = P;
  const dirX = focalX - x;
  const dirY = focalY - (y - 150 * s);
...
function companionFigure(kind: CompanionKind, x: number, y: number, s: number, focalX: number, P: Palette["colors"]): string {
  const [, DEEP, , LIGHT, PAPER, WARM, COOL] = P;
  const px = focalX - x >= 0 ? 3 : -3;
  const eyes = (ey: number) =>
    `<circle cx="-7" cy="${ey}" r="5.4" fill="${PAPER}"/><circle cx="7" cy="${ey}" r="5.4" fill="${PAPER}"/>` +
    `<circle cx="${R(-7 + px)}" cy="${ey}" r="2.6" fill="${DEEP}"/><circle cx="${R(7 + px)}" cy="${ey}" r="2.6" fill="${DEEP}"/>`;
  let body = "";
  switch (kind) {
    case "cat":
      body = `<ellipse cy="-34" rx="30" ry="26" fill="${WARM}"/><circle cy="-72" r="22" fill="${WARM}"/>` +
        ...
        eyes(-74) + `<path d="M-4 -64 Q0 -60 4 -64" fill="none" stroke="${DEEP}" stroke-width="2.6" stroke-linecap="round"/>` +
    ...
    case "dog":
      body = `<ellipse cy="-32" rx="30" ry="25" fill="${LIGHT}"/><circle cy="-70" r="22" fill="${LIGHT}"/>` +
        ...
        `<ellipse cx="20" cy="-80" rx="9" ry="16" fill="${DEEP}" transform="rotate(-18 20 -80)"/>` +
        eyes(-72) + `<ellipse cy="-58" rx="8" ry="6" fill="${DEEP}"/>` +
    ...
    case "bunny":
      body = `<ellipse cy="-32" rx="27" ry="25" fill="${PAPER}"/><circle cy="-68" r="20" fill="${PAPER}"/>` +
        ...
        eyes(-70) + `<circle cy="-60" r="3.4" fill="${WARM}"/>`;
      break;
    case "bear":
      body = `<ellipse cy="-34" rx="30" ry="27" fill="${COOL}"/><circle cy="-74" r="23" fill="${COOL}"/>` +
        ...
        eyes(-76) + `<ellipse cy="-64" rx="9" ry="7" fill="${PAPER}"/><circle cy="-66" r="3" fill="${DEEP}"/>`;
      break;
    case "robot":
    ...
    case "fox":
      body = `<ellipse cy="-32" rx="28" ry="24" fill="${WARM}"/><circle cy="-68" r="21" fill="${WARM}"/>` +
        ...
        `<ellipse cy="-58" rx="8" ry="6" fill="${PAPER}"/>` + eyes(-72) +
...
export function pickCompanionKind(input: BookInput, seed: number): CompanionKind | null {
  ...
  return pick(rnd, input.companion.type === "toy" ? (["bear", "robot"] as CompanionKind[]) : (["cat", "dog", "bunny", "fox"] as CompanionKind[]));
}
...
export function renderScene(spec: SceneSpec): string {
  ...
  const [, DEEP, , LIGHT] = P;
  const dark =
    spec.stage === "climax" || spec.stage === "trial" ? rnd() > 0.55 : spec.stage === "return_lesson" && rnd() > 0.5;
  const night = spec.genre === "space" || dark;
  const skyTop = night ? P[0] : DEEP;
  const skyBot = night ? DEEP : spec.genre === "underwater" ? P[2] : LIGHT;
  ...
  if (spec.companionKind) {
    ...
    parts.push(companionFigure(spec.companionKind, compX, cy + 6, 0.92, fx, P));
  }
...
export function renderCoverScene(input: BookInput, palette: Palette, seed: number, kind: CompanionKind | null): string {
  ...
  return renderScene({
    ...
    stage: "call_to_adventure", emotion: "восторг", focal: pick(rnd, GENRE_FOCALS[input.genre]),
    spreadNumber: 0, isCover: true,
  });
}
...
/** Промпт для реальных API-провайдеров (Gemini / YandexART) — тот же стиль и правила */
