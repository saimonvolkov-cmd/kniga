...
/* ── Export Module ─────────────────────────────────────────────────────────
   ...
   отдельным дизайн-элементом. Кириллица рендерится канвасом, поэтому в PDF
   не нужно встраивать шрифты. */
...
/** roundRect с фолбэком для старых браузеров */
...
function drawWrapped(
  ctx: CanvasRenderingContext2D, text: string, cx: number, cy: number, maxWidth: number, lineHeight: number
): number {
...
function pageCanvas(page: GeneratedPage, book: GeneratedBook, palette: Palette): Promise<HTMLCanvasElement> {
  ...
  return (async () => {
    ...
    if (page.kind === "cover") {
      ...
      rr(ctx, 90, topY - 70, PX - 180, lines.length * 104 + 84, 36);
      ctx.fill();
      ctx.globalAlpha = 1;
      ctx.lineWidth = 10;
      ctx.strokeStyle = INK;
...
export async function exportBookPdf(
  ...
  // точная копия байтов — целый ArrayBuffer для Blob
