...
import type {
  ApiKeys, BookInput, GeneratedBook, GeneratedPage, PipelineStageId, PipelineStepState, SurveyDraft,
} from "./types";
import { getPalette } from "./data/content";
import { generateStoryViaApi, loadKeys, saveKeys, generateImageViaApi } from "./lib/api";
import { moderateStory } from "./lib/safety";
import { KIND_NOUN } from "./lib/storyEngine";
import {
  buildImagePrompt, focalForSpread, pickCompanionKind, renderBackScene, renderCoverScene, renderScene,
...
const PIPELINE: Array<{ id: PipelineStageId; label: string }> = [
  ...
  { id: "cover", label: "Обложки — Cover Module (Gemini)" },
  { id: "spreads", label: "Развороты — Illustration Module (Gemini)" },
...
export default function App() {
  ...
  const stepState = (i: number): { canNext: boolean; hint?: string } => {
    switch (i) {
      ...
      case 5:
        ...
        return { canNext: true };
      case 6: return { canNext: true };
      case 7: return { canNext: true };
  ...
  const runPipeline = useCallback(
    async (inp: BookInput, activeSeed: number) => {
      ...
      /* 1 — история */
      setSt("story", "active");
      addLog(keys.anthropic ? "Narrative: вызываю Claude Sonnet…" : "Narrative: ключ Anthropic не задан — локальный движок");
      ...
      addLog(
        report.blocked
          ? `Safety: ${report.blocked} BLOCK → текст перегенерирован мягче`
          : `Safety: все ${report.checked} разворотов ALLOW`
      );
      ...
      let coverImg = await generateImageViaApi(coverPrompt, draft.childPhotos, keys);
      if (cancelled()) return;
      if (!coverImg) {
      ...
      const coverPage: GeneratedPage = { kind: "cover", image: coverImg };
      setLatest(coverPage);
      addLog(`Cover: передняя обложка готова (${coverImg.startsWith("<") ? "демо-SVG" : "Gemini"})`);
      ...
      let backImg = await generateImageViaApi(backPrompt, [], keys);
      if (cancelled()) return;
      if (!backImg) {
      ...
      const backPage: GeneratedPage = { kind: "back", image: backImg };
      addLog(`Cover: задняя обложка готова (${backImg.startsWith("<") ? "демо-SVG" : "Gemini"})`);
      setSt("cover", "done", "передняя + задняя");
      ...
      for (let i = 0; i < story.spreads.length; i++) {
        ...
        let img = await generateImageViaApi(prompt, draft.childPhotos, keys);
        if (cancelled()) return;
        if (!img) {
          const spec: SceneSpec = {
            ...
            stage: sp.stage,
            emotion: sp.emotion,
            focal: focalForSpread(inp.genre, sp),
            spreadNumber: sp.spread_number,
          };
          img = renderScene(spec);
          await delay(230 + ((i * 53) % 190));
          if (cancelled()) return;
        }
        const page: GeneratedPage = { kind: "spread", image: img, spread: sp };
        acc.push(page);
        setPages([...acc]);
        setLatest(page);
        addLog(`Illustration: разворот ${sp.spread_number} · ${sp.stage} · ${sp.characters_present.join("+")} (${img.startsWith("<") ? "SVG" : "Gemini"})`);
      }
      acc.push(backPage);
      setPages([...acc]);
      setLatest(backPage);
      setSt("spreads", "done", `${story.spreads.length} иллюстраций по scene_description`);
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = pdfFileName(book.story.title);
      a.click();
      window.setTimeout(() => URL.revokeObjectURL(url), 4000);
      toast("ok", `PDF сохранён: ${pdfFileName(book.story.title)}`);
    } catch (e) {
      console.error(e);
      toast("err", "Не удалось собрать PDF — попробуйте ещё раз");
    } finally {
      setExporting(null);
    }
  }, [book, toast]);
  ...
  const apiMode = Boolean(keys.gemini || keys.anthropic);
    ...
    <div className="relative min-h-screen">
      ...
      <SettingsModal
        ...
        onSave={(k) => {
          setKeys(k);
          saveKeys(k);
          toast("ok", k.gemini || k.anthropic ? "Ключи сохранены — пайплайн пойдёт через Gemini/Claude" : "Ключи очищены — включён демо-режим");
        }}
      />
      ...
      <header className="sticky top-0 z-40 border-b-[3px] border-ink bg-mist/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <button
            ...
            onClick={() => { setPhase("survey"); setStep(0); }}
          >
            ...
            <span>
              ...
              <span className="mt-0.5 block text-[10.5px] font-extrabold uppercase tracking-[0.14em] text-ink/50">
                персональная книга · MVP
              </span>
            </span>
          </button>
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              {apiMode ? "API подключены" : "Демо-режим"}
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-4 pb-20 pt-8">
        {phase === "survey" && (
          <div className="grid gap-8 lg:grid-cols-[340px_1fr]">
            {/* левая колонка: живая обложка + главы */}
            ...
            <section key={step} className="card-paper step-enter h-fit p-6 sm:p-8">
              ...
              {step === 8 && !input && (
                <div className="py-10 text-center">
                  <p className="font-display text-lg font-bold text-pine">Черновик стал неполным</p>
                  <p className="mt-1 text-sm font-semibold text-ink/60">Вернитесь на шаг «Герой» и заполните обязательные поля.</p>
                  <ChunkyButton className="mt-5" onClick={() => setStep(1)}>К шагу 2</ChunkyButton>
                </div>
              )}
              {step < 8 && (
                <StepNav
