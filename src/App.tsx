...
import type {
  ApiKeys, BookInput, ConnMode, GeneratedBook, GeneratedPage, ImageSource,
  PipelineStageId, PipelineStepState, SurveyDraft,
} from "./types";
import { getPalette } from "./data/content";
import {
  detectConnection, generateIllustration, generateStoryViaApi, isQuotaError,
  loadKeys, resetQuotaBreaker, saveKeys, type ImageCallResult,
} from "./lib/api";
...
import { cx, Toasts, type ToastItem } from "./components/ui";
...
const PIPELINE: Array<{ id: PipelineStageId; label: string }> = [
  { id: "story", label: "История — Narrative Module" },
  { id: "safety", label: "Модерация — Content Safety" },
  { id: "reference", label: "Референс персонажа — Character Reference" },
  { id: "cover", label: "Обложки — Cover Module" },
  { id: "spreads", label: "Развороты — Illustration Module" },
...
export default function App() {
  ...
  /** настроен ли Yandex на локальном прокси */
  const [connMode, setConnMode] = useState<ConnMode | "checking">("checking");

  const runToken = useRef(0);
  const toastId = useRef(0);
  /** ref на карточку текущего шага — для плавного подскролла без прыжка наверх */
  const cardRef = useRef<HTMLElement | null>(null);
  /** счётчик рендеров шага: первый (первая загрузка) не скроллим */
  const stepRenderCount = useRef(0);
  ...
  /* ── СКРОЛЛ: фикс главного бага ─────────────────────────────────────────
     Раньше: useEffect(scrollTo top, [phase, step]) — каждый переход шага
     дёргал страницу в самый верх. Теперь:
     · смена ЭКРАНА (phase) — наверх, это новая сцена;
     · смена ШАГА — карточка плавно приводится в зону видимости, и только если
       она реально вне «комфортной зоны» (иначе не дёргаем вообще);
     · при выборе ОТВЕТА (patch состояния) скролла нет вообще: состояние
       обновляется без пересоздания карточки (key не меняется). */
  useEffect(() => {
    window.scrollTo({ top: 0 });
    stepRenderCount.current = 0; // новая фаза — снова разрешаем «первый рендер без скролла»
  }, [phase]);

  useEffect(() => {
    if (phase !== "survey") return;
    stepRenderCount.current += 1;
    if (stepRenderCount.current <= 1) return; // первая загрузка — не скроллим
    const el = cardRef.current;
    if (!el) return;
    const r = el.getBoundingClientRect();
    const inComfort = r.top >= 72 && r.top <= window.innerHeight * 0.6;
    if (inComfort) return; // карточка уже на виду — никаких прыжков
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    el.scrollIntoView({ behavior: reduce ? "auto" : "smooth", block: "start" });
  }, [step, phase]);

  /* живой опрос Yandex-прокси: пока не найден — спрашиваем каждые 4 с,
     при запуске сервера индикатор зеленеет сам, без перезагрузки */
  useEffect(() => {
    let stop = false;
    ...
    const probe = async () => {
      ...
      if (stop) return;
      setConnMode((prev) => {
        if (prev !== "backend" && mode === "backend") toast("ok", "Yandex-прокси найден — провайдер подключён");
        return mode;
      });
  ...
  const runPipeline = useCallback(
    async (inp: BookInput, activeSeed: number) => {
      ...
      const viaLabel: Record<string, string> = {
        gemini: "Gemini", "yandex-art": "YandexART", huggingface: "Hugging Face", pollinations: "Pollinations",
      };
      let firstErr: string | null = null;
      const noteImage = (r: ImageCallResult) => {
        if (r.dataUrl) return;
        addLog(`${viaLabel[r.via ?? "gemini"] ?? "Gemini"}: ${r.error}`);
        if (!firstErr) {
      ...
      const sourceOf = (r: ImageCallResult): ImageSource => (r.dataUrl ? ((r.via as ImageSource) ?? "gemini") : "demo");
      const labelOf = (r: ImageCallResult): string => (r.dataUrl ? viaLabel[r.via ?? "gemini"] ?? "Gemini" : "демо-SVG");
      ...
      /* 1 — история */
      setSt("story", "active");
      addLog(connMode === "backend"
        ? "Narrative: история через прокси /api/yandex/generate-text (YandexGPT, ключ на сервере)"
        : "Narrative: вызываю текстовых провайдеров (Claude/Gemini/демо)…");
      ...
      const coverPage: GeneratedPage = {
        kind: "cover", image: coverImg,
        imageSource: sourceOf(coverRes), imageError: coverRes.dataUrl ? undefined : coverRes.error ?? undefined,
      };
      setLatest(coverPage);
      ...
      const backPage: GeneratedPage = {
        kind: "back", image: backImg,
        imageSource: sourceOf(backRes), imageError: backRes.dataUrl ? undefined : backRes.error ?? undefined,
      };
      addLog(`Cover: задняя обложка готова (${labelOf(backRes)})`);
      setSt("cover", "done", `передняя: ${labelOf(coverRes)} · задняя: ${labelOf(backRes)}`);
      ...
      for (let i = 0; i < story.spreads.length; i++) {
        ...
        const page: GeneratedPage = {
          kind: "spread", image: img, spread: sp,
          imageSource: sourceOf(res), imageError: res.dataUrl ? undefined : res.error ?? undefined,
        };
        acc.push(page);
        setPages([...acc]);
        setLatest(page);
        addLog(`Illustration: разворот ${sp.spread_number} · ${sp.stage} · ${sp.characters_present.join("+")} (${labelOf(res)})`);
      }
      acc.push(backPage);
      setPages([...acc]);
      setLatest(backPage);
      ...
      const generated: GeneratedBook = {
        ...
        imageReport: {
          gemini: geminiDrawn, yandex: yandexDrawn, hf: hfDrawn, pollinations: polDrawn,
  ...
  const copyServerCmd = useCallback(() => {
    ...
    if (navigator.clipboard?.writeText) {
      ...
      toast("warn", `Запустите в терминале: ${cmd}`);
    }
  }, [toast]);
  ...
  const connMeta: Record<ConnMode | "checking", { label: string; title: string; pill: string; dot: string }> = {
    ...
    backend: {
      ...
      title: "YandexGPT и YandexART идут через локальный прокси — ключ в серверном .env, в браузер не попадает",
      pill: "bg-pine text-marigold",
      dot: "bg-marigold",
    },
    off: {
      label: "Yandex не настроен",
      title: "Нажмите, чтобы скопировать команду запуска прокси: npm --prefix server run server (ключи — из серверного .env)",
    ...
    <div className="relative min-h-screen">
      ...
      <header className="sticky top-0 z-40 border-b-[3px] border-ink bg-mist/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              onClick={() => (connMode === "off" ? copyServerCmd() : undefined)}
              title={connMeta[connMode].title}
              className={cx(
                "btn-press flex items-center gap-2 rounded-xl border-[2.5px] border-ink px-3 py-1.5 font-display text-[12px] font-bold shadow-block-sm",
            ...
            <button
              ...
              title="Ключи Gemini / Hugging Face / Anthropic (Yandex — на сервере)"
              className="btn-press flex items-center gap-2 rounded-xl border-[2.5px] border-ink bg-paper px-3 py-1.5 font-display text-[12.5px] font-bold text-pine shadow-block-sm"
            >
              <IconGear className="h-4 w-4" />
              <span className="hidden md:inline">Ключи</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-4 pb-20 pt-8">
        {phase === "survey" && (
          <div className="grid gap-8 lg:grid-cols-[340px_1fr]">
            ...
            <section key={step} ref={cardRef} className="card-paper step-enter h-fit scroll-mt-24 p-6 sm:p-8">
              ...
              {step === 8 && !input && (
                <p className="font-display text-[15px] font-bold text-coral">
                  Черновик неполный — вернитесь на шаг 2 и заполните имя, возраст и пол.
                </p>
              )}
              {step < 8 && (
                <StepNav
      ...
      <footer className="relative z-10 border-t-[3px] border-ink bg-foam/80 py-5">
        <p className="mx-auto max-w-6xl px-4 text-center text-[12px] font-bold text-ink/50">
          MVP-пайплайн: Survey → Narrative → Safety → Reference → Cover → Illustration → Layout → Export ·
          Yandex — через локальный прокси, ключ в серверном .env
        </p>
      </footer>
    </div>
  );
