...
/* 1) YandexGPT — синхронный completion (yandexgpt-lite) */
console.log("1) YandexGPT · yandexgpt-lite · синхронный completion…");
const t0 = Date.now();
...
/* 2) YandexART — асинхронная генерация + поллинг */
...
console.log(
  `✓ YandexART нарисовал картинку за ${((Date.now() - t1) / 1000).toFixed(1)} с → ${out} (${Math.round(
    (b64.length * 0.75) / 1024
  )} КБ)`
);

console.log("\nУСПЕХ: ключ и роли работают.");
console.log("Дальше:  node server/index.js   → приложение на http://127.0.0.1:8787");
